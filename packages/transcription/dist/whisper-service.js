...
export class WhisperTranscriptionService {
    ...
    async transcribe(audio) {
        ...
        try {
            ...
            const transcriptionText = Array.isArray(result)
                ? result.map((segment) => segment.speech).join(' ')
                : String(result);
    ...
    async audioBufferToWav(audio) {
        // Create a simple WAV file header
        const wavHeader = this.createWavHeader(audio.data.byteLength, audio.sampleRate, audio.channels);
